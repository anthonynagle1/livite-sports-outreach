...
def _fetch_open_meteo(date_str):
    ...
    return {
        ...
        "temp_avg": _first("temperature_2m_mean"),
        "precipitation_inches": _first("precipitation_sum", 0) or 0,
        "rain_inches": _first("rain_sum", 0) or 0,
        "snow_inches": _first("snowfall_sum", 0) or 0,
        "wind_max_mph": _first("windspeed_10m_max"),
        "wind_gust_mph": _first("windgusts_10m_max"),
...
def get_daily_weather(date):
    ...
    try:
        ...
        logger.warning("Weather fetch error for %s: %s", date_str, e)
        weather = None
    ...
    try:
        ...
        logger.warning("Sunset fetch error for %s: %s", date_str, e)
...
def _fetch_open_meteo_forecast(date_str):
    ...
    return {
        ...
        "temp_avg": (daily.get("temperature_2m_mean") or [None])[0],
        "precipitation_inches": (daily.get("precipitation_sum") or [0])[0] or 0,
        "rain_inches": (daily.get("rain_sum") or [0])[0] or 0,
        "snow_inches": (daily.get("snowfall_sum") or [0])[0] or 0,
        "wind_max_mph": (daily.get("windspeed_10m_max") or [None])[0],
        "wind_gust_mph": (daily.get("windgusts_10m_max") or [None])[0],
        "weather_code": weather_code,
        "conditions": WMO_CODES.get(weather_code, "Unknown"),
        "sunrise": _format_time(sunrise_raw),
        "sunset": _format_time(sunset_raw),
...
def get_today_weather():
    ...
    try:
        ...
        logger.warning("Forecast weather fetch error for %s: %s", date_str, e)
        weather = None
...
def warm_weather_cache(earliest_str="20241107"):
    ...
    try:
        ...
        logger.warning("Bulk weather fetch error: %s", e)
        return 0, total_days
