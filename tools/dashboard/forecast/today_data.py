...
def generate_today_prediction():
    """Generate today's operational prediction.

    Returns dict with weather, prediction, hourly curve, peaks,
    dayparts, scheduled catering, this-week context, and staffing hints.
    """
    today = datetime.now()
    date_iso = today.strftime("%Y-%m-%d")
    date_yyyymmdd = today.strftime("%Y%m%d")

    logger.info("Generating today's prediction for %s (%s)...",
                date_iso, _DOW_FULL[today.weekday()])
    ...
    # Legacy single-model (kept for explanation baseline)
    ...
    rev_instore = _forecast_channel_day(today, "instore", ch_trends, ch_dow, ch_seasonal)
    rev_delivery = _forecast_channel_day(today, "delivery", ch_trends, ch_dow, ch_seasonal)
    # BOGO adjustment: model is trained on mostly pre-BOGO data, haircut delivery
    ...
    # ── Scheduled catering ──
    catering_orders = _get_scheduled_catering(date_iso)
    catering_boost = sum(o.get("subtotal", 0) for o in catering_orders)

    # ── Final prediction (weather applies to in-store + delivery, not catering) ──
    ...
    # ── Hourly distribution ──
    hourly, quarter_hourly, weeks_found = _build_hourly_curve(today)

    # Scale hourly curve to match final prediction
    ...
    explanation = _build_explanation(
        today, trend, dow_indices, seasonal_indices,
    ...
    this_week = _build_this_week(
        today, df, trend, dow_indices, seasonal_indices,
        channel_mix, expense_ratios,
    ...
    logger.info(
        "Today's prediction: $%s (In-Store $%s, Delivery $%s, Catering $%s) "
        "weather=%.2f, sched_catering=$%s",
        "{:,.0f}".format(final_prediction),
        "{:,.0f}".format(rev_instore_final),
        "{:,.0f}".format(rev_delivery_final),
        "{:,.0f}".format(rev_catering_final),
        weather_mult,
        "{:,.0f}".format(catering_boost),
    )
