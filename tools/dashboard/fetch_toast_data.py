...
def _get_container_client():
    """Lazy-initialize Azure container client (avoids import-time crash when env var is missing)."""
        ...
        for future in as_completed(futures):
            try:
                ...
                if not quiet:
                    ...
                    logger.debug("  + %s (%s)", fname, tag)
            except Exception as e:
                fname = futures[future]
                logger.warning("Failed to fetch %s: %s", fname, e)

    if not quiet and cached_count > 0:
        logger.debug("  (%d/%d from cache)", cached_count, len(DAILY_FILES))
    ...
    while current <= end_date:
        ...
        if day_data:
            ...
            if not quiet:
                ...
                logger.debug("    %d files loaded", fetched)
        current += timedelta(days=1)

    if not quiet:
        logger.debug("  Total: %d day(s) loaded", len(all_days))
...
def list_available_dates() -> list:
    ...
    blobs = _get_container_client().list_blobs()
    dates = set()
    for blob in blobs:
...
def get_date_range(start: str, end: str, filename: str = "OrderDetails.csv") -> pd.DataFrame:
    ...
    while current <= end_dt:
        ...
        try:
            ...
            logger.warning("get_date_range: skipping %s for %s: %s", date_str, filename, e)
        current += timedelta(days=1)
